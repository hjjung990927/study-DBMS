...
/* insert */
/* 회원 3명 추가 */
insert into tbl_member (member_id, member_name, member_password, member_age)
values('1', '홍길동', '1', '20');
insert into tbl_member (member_id, member_name, member_password, member_age)
values('2', '이순신', '2', '21');
insert into tbl_member (member_id, member_name, member_password, member_age)
...
/* 1번 -> 2번 좋아요 */
insert into tbl_like(like_sender, like_receiver)
values('1', '2');
...
from tbl_user u join tbl_order o
on u.id = o.user_id
join tbl_product p
...
/* 상품명에 v가 포함된 상품 검색 */
select * from tbl_product
where product_name like concat('%', 'v', '%');
...
insert into tbl_owner (owner_name, owner_age, owner_phone, owner_address)
values
...
insert into tbl_pet (pet_ill_name, pet_name, pet_age, pet_weight, owner_id)
values
('장염', '뽀삐', 4, 10.45, 1),
('감기', '달구', 12, 14.25, 2),
('탈골', '댕댕', 7, 8.46, 2),
...
select avg(p.pet_age), max(o.owner_age), p.pet_ill_name 
from tbl_pet p
join tbl_owner o on p.owner_id = o.id
...
alter table tbl_child add (child_name varchar(255) not null);
...
insert into tbl_parent (parent_name, parent_address, parent_phone)
values
('한동석', '경기도', '01012341234'),
('홍길동', '서울', '01012341235'),
('이순신', '대구', '01012345654'),
 ...
 * 
 * from: 인라인 뷰
 * where: 서브 쿼리 -> 조회 결과의 컬럼 갯수와 where절의 컬럼갯수가 같아야한다.
 * select" 스칼라 서브 쿼리 -> select절에서 컬럼 1개만
*/
...
/*20대 중에서 구매 가격이 300,000원을 넘는 주문번호, 상품가격, 연령대 조회*/
select o.id, p.product_price, u.user_age 
from tbl_order o
join tbl_product p on o.product_id = p.id 
join tbl_user u on o.user_id = u.id
where u.user_age between 20 and 29 and p.product_price > 300000
group by o.id;
...
insert into tbl_member (member_id, member_password, member_age, member_name)
values('test1234', '1234', 10, '홍길동');
insert into tbl_member (member_id, member_password, member_age, member_name)
values('test5555', '1234', 20, '이순신');
insert into tbl_member (member_id, member_password, member_age, member_name)
